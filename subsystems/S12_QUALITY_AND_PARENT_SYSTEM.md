...
| v1.3 | 2026-05-19 | 人审修复（N1-N6全部已由v1.2前置修复覆盖）。审计P1-ref修复：第765行S10跨系统引用补全锚定（S10 §二、Phase 1 事件（完整 Schema）——事件 1/4/8/9/10）。版本v1.2→v1.3。 |
| v1.4 | 2026-05-19 | P1-ref修复：S10跨系统引用改写为自包含依赖声明，移除「尚未封版」终结态违规词，引用锚定S10 §二 Phase 1 事件（事件1/4/8/9/10）+ Sync Hook。版本v1.3→v1.4。 |
...
|| 禁止词过滤 | P0 | 完整文本生成后扫描 18 个禁止词 + 7 种禁止表达模式，检出率 100% |
| 风险评分计算 | P0 | 6 项风险因素计算 riskScore ∈ [0,1]，riskScore ≥ 0.5 不发送进人工复审 |
|| 家长反馈闭环 | P0 | 5 种反馈准确记录至 ParentSummary.parentFeedback（accurate/inaccurate/helpful/unsuitable/reobserve），其中 reobserve 不参与 trustScore 计算<!-- AI-MUTABLE: trustScore初始值, type=float, range=[0.3, 0.7] --> |
...
| 请芽芽重新观察 | reobserve | 触发重诊断（ignoreCache=true），写入 parentFeedback 字段但不参与 trustScore 计算 |
...
| 4 | 策略包覆盖率 | 已实现策略包 / 定义策略包 | 0/40 | Phase 1: 13/40 = 32.5% |
...
红色指标自动进入 top 10 待修。其余 17 项指标为仅展示指标，不设红黄绿告警阈值。
...
| diagnosis_updated 事件未到达 | shortSummary 使用缓存的最后有效值→摘要中显示"芽芽正在分析" | 事件恢复后下次摘要自动更新 |
| answer_submitted 事件未到达 | fullSummary 中不包含具体题目描述→使用通用措辞 | 事件恢复后下次摘要自动更新 |
| strategy_applied 事件未到达 | fullSummary 中不包含策略描述→跳过策略部分 | 事件恢复后下次摘要自动更新 |
| strategy_completed 事件未到达 | riskScore 计算跳过 effectScore 因子→使用剩余 5 项因子 | 事件恢复后下次摘要自动更新 |
...
| 卡点 | S12 对应位置 | 失败行为 |
|------|-----------|---------|
| G1 | 摘要生成/仪表盘计算前（§一目标 + §八目标）—不可修改边界检查 | 拒绝生成/计算 |
| G2 | 阶段推进前消费清单完整（§五消费关系清单全部 13 条闭合） | 拒绝推进 |
| G3 | 元进化——修改进化路线图维度前（§十七） | 拒绝元进化 |
| G4 | commit 前文档一致性检查（指标计数/禁止词数/大白话数/跨系统引用验证） | CI 阻断 |
| G5 | 摘要模板变更/风险评分规则变更代码审查（由审查员审核北极星信号影响） | 阻断未审查的内容变更 |
| G6 | 新摘要模板/新阈值进入影子模式前（§安全执行管道第一步） | 拒绝进入影子 |
| G7 | 影子→A/B 前（PSI < 0.1 + 北极星底线信号无恶化） | 拒绝进入 A/B 测试 |

- G1-G7 七卡点（§G1-G7 卡点预埋表）✔
- 大白话 9 类：已覆盖（家长摘要引擎/质量仪表盘/自我进化/不可修改边界/消费关系/异常处置/应急熔断/进化信用分/内容与代码进化分离）✔
- 消费清单 7 列完整 ✔
- 被消费方待定义标注 ✔
- S12 依赖事件：S12 消费的 5 种事件（lesson_completed/diagnosis_updated/strategy_applied/strategy_completed/answer_submitted）Schema 基于 S10 v1.6 §二 Phase 1 事件（事件 1/4/8/9/10）。S12 已给出全部 5 种事件缺失时的降级方案（见§降级策略），所有依赖指标的 Phase 1 兜底值为 null 或冷启动值。当 S10 事件 Schema 发生不兼容变更时，需同步更新 S12 降级策略。✔
