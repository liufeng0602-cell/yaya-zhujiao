...
- 消费清单 7 列完整 ✔
- 被消费方待定义标注 ✔
- S10 尚未封版风险标注：S12 依赖的 5 种事件（lesson_completed/diagnosis_updated/strategy_applied/strategy_completed/answer_submitted）Schema 当前由 S10 v1.6 定义，S10 尚未封版。S12 已给出全部 5 种事件缺失时的降级方案（见§降级策略），所有依赖指标的 Phase 1 兜底值为 null 或冷启动值。待 S10 封版后重新验证事件 Schema 兼容性。✔
