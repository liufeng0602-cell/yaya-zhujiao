...
status 与 diagnosisStatus 的对照（六态。注：S07 诊断引擎曾存在「七态」与「六种状态」的自相矛盾——S07 v1.10 已修复，S10 侧当前定义正确，等待 S07 完全对齐后交叉验证）：
...
86 错因全量分布：L1 通用 10 个 + L2 数学特有 76 个。数据文件：error_taxonomy.json。
