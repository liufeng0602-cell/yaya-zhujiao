...
| 字段 | 用途 |
|------|------|
| answerTimeMs | 过快（< 3 秒）→ 猜测信号；过慢（> 3σ）→ 挣扎信号 |
...
isCorrect = true   → 默认跳过，但以下三个例外强制启动：
  1. answerTimeMs > 该知识点历史均值的 3 倍标准差（异常慢，可能蒙对）
...
| isCorrect=false 且 hintUsed=true, hintLevel=3 | 全提示仍错 → 概念层面问题，调整先验概率 |
...
| 模板后仍不会 → 不判"题型不熟"，改判概念/策略问题 | 看了模板还不会，不是题型的问题 |
...
如果 status = "open" 且 mainHypothesis.confidence ≥ 0.5：
  ...
  选择对应的验证题（errorVerificationQuestions 库）
...
**示例**：
答错"分数除法应用题" → 主假设全部排除 → 追溯前置"分数除法计算" → 掌握度 0.3 → 主假设改为"分数除法计算不熟练" → 触发验证。
...
| 条件 | 行为 |
|------|------|
| 该知识点下学生答题 < 30 条 | 置信度自动打折 × 0.7。禁止触发强策略 |
| 该知识点全局答题 < 120 条 | 该知识点的 error_kp_bindings 权重降低，多依赖 L1 通用错因 |
| 学生画像中该知识点的邻居节点（前置/后续）数据充足 | 借用邻居的错因分布作为先验 |
...
mainHypothesis.confidence < 0.5 → 不触发验证题
  ...
  策略引擎（12 号文档）消费 recommendedAction 字段执行实际操作（降低难度、推送提示）。诊断引擎不负责出题，不直接操作题目难度。
```

低置信度错因绝不能触发强策略（如变式特训包、精准计算包）。只能触发轻量安全策略（基础巩固包或记忆唤醒包）。
...
inconclusive 的后续动作（诊断引擎发出建议，策略引擎执行）：
1. 诊断引擎在 diagnosis_updated 中设置 recommendedAction = "provide_scaffolding"
...
**parentSummary 生成规则**：
- 使用 error_taxonomy.json 中该错因的 parentSafeExplanation 作为母版
...
| parentSafeExplanation | 家长中性解释 | parentSummary.short 母版 |
...
引擎使用方式：validation_triggered 事件中携带 validationQuestionId，从该题库选取。
...
| 文档 | 依赖内容 |
|------|----------|
| 09_EVENT_PROTOCOL.md (v2.0.2) | answer_submitted 事件格式（含三层结构、chainId、三例外规则）；diagnosis_updated 事件 Schema（事件 8） |
...
| 文档 | 输出内容 |
|------|----------|
| 09_EVENT_PROTOCOL.md (v2.0.2) | diagnosis_attempted 事件（事件 5）、validation_triggered 事件（事件 6）、diagnosis_updated 事件（事件 8） |
...
- [ ] 冷启动时（该知识点答题 < 30 条），confidence 打折，不强策略
