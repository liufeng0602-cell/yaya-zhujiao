...
系统支持最多 3 个 API Key 同时配置，每个 Key 提供 2 个并发槽位，合计最大 6 个独立任务并行执行。

配置要点：
- 每个项目只需 1 个 Key——同一项目内 Writer 和 Reviewer 是串行工作的，不争抢并发槽位
- 跨项目并行时，3 个项目同时有 Writer 工作（3 个并发）或同时有 Reviewer 工作（3 个并发），2 个 Key（4 槽位）已足够覆盖
- 第 3 个 Key 作为冗余备用或应对突发批量任务
