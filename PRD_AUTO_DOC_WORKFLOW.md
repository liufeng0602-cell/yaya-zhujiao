# 自动化文档生产-审核循环工作流 PRD v2.12

## 变更记录

| 版本 | 日期 | 变更内容 |
|------|------|----------|
||| v2.12 | 2026-05-19 13:28 | MAX_CONCURRENCY 3→2（DeepSeek 突发保护限制，gateway.log 已记录22次429）；卡片三状态机(运行中/排队中/已停止)取代二态徽标；人工审核不通过退回修改时递增迭代数 |
||| v2.11 | 2026-05-19 13:04 | 并行架构改造 v3.0: fswatch异步spawn(Popen)+并发上限(目标3/架构5)、writer.py批量处理(原子CAS claim)、git commit --<file>隔离、NOTIFY按task_id独立文件、kanban_ops加busy_timeout+try_claim_task |
||| v2.10 | 2026-05-19 12:48 | 修复复审不通过列跳转bug：复审不通过后卡片应停留复审+修改区且提示改为「5秒钟之后继续修改」；增加revision状态来自re_review时永久归属复审+修改列规则 |
||| v2.9 | 2026-05-19 12:37 | 修复过渡中卡片列位置bug：所有状态切换时卡片应停留在原列展示5秒倒计时，倒计时结束再跳转至新列 |
|| v2.8 | 2026-05-19 12:25 | 修复复审超时bug：人工审核不通过直接进revision而非re_review（Writer扫描目标） |
...
- 第2步：按模板写初稿
- 第3步：自检——自检查"有没有写"（7项必备内容是否都有覆盖），不是检查"写得对不对"（那是reviewer的事）。执行硬编码 grep 清单（见下）。自检发现的 P0/P1 问题必须修复后才能提交；P2 问题允许记录但不阻塞提交。
...
**needs_revision 状态下的修改规则：**
- Writer 必须修复 Reviewer 标记的所有 P0/P1 问题
- Writer 也可以修复自己发现的额外问题（不限于 Reviewer 指出的范围）
- 复审提交时，必须在自检声明中用 git diff 单独列出"额外修改内容"，让 Reviewer 在下一轮全量审核中重点检查这些区域
- 修复完成后重新执行生产流程第3步（自检）和第4步（版本号 bump + 变更记录），然后提交审核
...
**审核覆盖范围（8项为最低必审标准，不限于8项。审核P发现任何8项之外的文档质量问题，也必须标注并输出）：**
...
- blocked（max_iterations_exceeded）：连续6轮 needs_revision 仍无法 passed

**审核方式：全量审核。**
Reviewer 每次收到 awaiting_review 的文档，从头到尾完整审一遍，不限于增量标记。上线后观察 token 消耗情况，后续可优化审核策略降低消耗。
...
**Writer Profile (yaya)**
- 职责：扫描 kanban board，发现 backlog 或 revision 的 task 时启动写作
- 触发方式：fswatch 守护进程检测到 NOTIFY 文件后调用 `hermes chat -q`
- 写作流程：读需求->写文档->自检（含 wrapper.py 调 Vale/markdownlint）->git commit->NOTIFY 触发 reviewer->更新 kanban
- 人工反馈闭环：当任务包含 human_feedback 时，先逐条分析(属实/半属实/不属实) -> 修改文档 -> 格式模板自查 -> 执行标准流程
- Writer 自检时自动调 wrapper.py 跑语法检查
- 自动修复：自检失败或 git commit 失败时最多重试 3 次，3 次后按状态推进（revision 任务继续重试，backlog 任务送审）
- **权限边界**：写权限：projects/ 目录、audit-reports/ 目录、.kanban/。读权限：evolution_rules.yaml。无权限：修改 reviewer 的配置或记忆

**Reviewer Profile (yaya-reviewer)**
- 职责：被 fswatch 守护进程触发后扫描 awaiting_review/re_review 任务，按 updated_at 最旧优先处理
- 模型：deepseek-v4-pro（与 writer 的 deepseek-reasoner 完全隔离）
- 记忆和技能独立于 writer
- 审核流程：读文档->按审核覆盖范围检查（8项为最低标准，不限于8项）->写审计报告->更新 kanban 状态
- 复审流程：检测到 re_review 优先处理 -> claim(re_reviewing) -> 全量审核 -> 判定（不通过→revision，通过→waiting_human_review）
- 复审后的 scope 自查：检查 human_feedback 是否在审核规则覆盖范围外，写出 reviewer_scope_notes.md
- 文档进入 waiting_human_review 后等待 liufeng 人工评审
- **权限边界**：读权限：projects/ 目录下的被审核文档。写权限：audit-reports/（写审计报告）、.kanban/。无权限：修改文档文件本身。发现错别字或格式问题，标注 P2 让 writer 改

**Watcher Profile (yaya-watcher)**
- 职责：已合并入 fswatch 守护进程。不再使用独立 Python watchdog watcher。
- fswatch 守护进程 (`scripts/fswatch_daemon.py`) 承担所有监控职责：
  - NOTIFY 目录监听：fswatch 二进制触发，零 token 空转
  - 自动化控制状态读取：读取 `.kanban/.control/automation_state.json`（停止/暂停/运行）
  - 定时 bootstrap（每60秒）：自动检测滞留任务、超时 re_review 迁移、补发 NOTIFY
  - 进程触发：检测到 writer-* NOTIFY 则启动 Writer（`hermes chat --profile yaya`），检测到 review-* NOTIFY 则启动 Reviewer（`hermes chat --profile yaya-reviewer`）
  - human_feedback 截断：超过 500 字符/3 项时自动截断
- 无独立 watcher 实例，fswatch 守护进程通过 `launchctl` 管理自动重启
- 启动/停止/暂停通过 Dashboard 控制按钮控制（写入 automation_state.json）
- **权限边界**：读权限：.kanban/（读 task 状态 + control文件）。写权限：.notify/（写 NOTIFY 文件）。执行权限：调用 hermes chat 触发 Writer/Reviewer
...
```
backlog -> drafting -> awaiting_review -> reviewing -> revision -> re_review -> re_reviewing -> waiting_human_review -> signed_off
                ↑                           ↑               ↑                              ↑
           writer开始写                 reviewer审不过  复审不通过                      复审通过
           (6轮上限)                    writer修完                                     等待人工评审
                                        (revision)

revision -> waiting_human_review（P0=P1=0 且 P2=0，直接跳复审，从 revision 到 waiting_human_review）
blocked <- 任意状态（watcher/fswatch发现异常/6轮上限/token超支）
```

| 状态 | 含义 | 谁处理 | 触发条件 |
|------|------|--------|----------|
| backlog | 待写作 | writer | liufeng 创建 task 或 fswatch 扫描发现 |
| drafting | 写作中 | writer | writer claim 后立即设置 |
| awaiting_review | 等待首次审核 | reviewer | writer 完成 + 验证通过 |
| reviewing | 审核中 | reviewer | reviewer claim 后立即设置 |
| revision | 需要修改（合并旧 needs_revision + p2_clearing） | writer | reviewer 判定不通过 (P0>0 或 P1>0) 或仅有 P2 |
| re_review | 复审中（Writer 已修完，等待 Reviewer 复查） | reviewer | writer 修改完成 + 提交 |
| re_reviewing | 复审中 | reviewer | reviewer claim 后立即设置 |
| waiting_human_review | 审核通过，等待人工评审（替代旧 approved） | **liufeng 手动** | reviewer 判定通过 (P0=P1=0) |
| signed_off | 封版 | **liufeng 手动** | 人工评审通过 + liufeng 终审签字 |
| blocked | 卡死 | fswatch / liufeng | 异常 / 6轮上限 / token 超支 |
...
| 6轮循环上限 | 回退到 backlog，重置 iteration_count=0，liufeng 补充需求说明后重开 | 需求和文档质量差距过大，需要重新对齐 |
| token 超支 | 回退到 drafting，调整 tokens_budget（x1.5或更多），继续写作 | 预估算少了，调完预算继续 |
| 数据丢失 | git checkout 恢复或回退到 drafting 重新写 | 看能否从 git 恢复 |
| 连续6轮审核不过 | 回退到 backlog，liufeng 判断是需求问题还是质量太差 | 需要人工决定是改需求还是改写法 |
...
Writer 完成写作并验证通过后，写 NOTIFY 文件到 `.kanban/.notify/review-<project>`。fswatch 守护进程检测到文件后触发 Reviewer。纯文件 IO，零 token。

### 6.2 Reviewer -> Writer 通知

Reviewer 判定不通过时，写 NOTIFY 文件到 `.kanban/.notify/writer-<project>`。fswatch 守护进程检测到文件后触发 Writer。

### 6.3 fswatch 守护进程通知

fswatch 守护进程监听 `.kanban/.notify/` 目录，接收到 NOTIFY 文件后：
- writer-* → 调用 `hermes chat -q "<prompt>" --profile yaya`
- review-* → 调用 `hermes chat -q "<prompt>" --profile yaya-reviewer`
- 不支持的命名 → 跳过并清除文件
- 处理完毕后清除 NOTIFY 文件

NOTIFY 内容示例：
```
NOTIFY by writer at 2026-05-18T22:33:09.123456
```
...
fswatch 二进制（零 token）
    ↓ 文件事件
fswatch 守护进程 scripts/fswatch_daemon.py
    ├── 60s 定时 bootstrap → 扫描 kanban → 写 NOTIFY
    ├── handle_notify → hermes chat --profile yaya/yaya-reviewer
    ├── 自动化控制状态 → 读取 .kanban/.control/automation_state.json
    ├── 滞留 re_review 迁移 → >5min re_review_result='fail' → revision
    ├── human_feedback 截断 → max 500 chars × 3 items
    └── 启动时处理遗留 NOTIFY 文件

Dashboard（port 9119, uvicorn）
    ├── 看板卡片列表（按状态分组）
    ├── 自动化控制按钮（▶ 运行 / ⏸ 暂停 / ⏹ 停止）
    ├── Profiles 状态指示（跟随工作流状态）
    ├── 卡片详情弹窗（P级分布、自动修复状态、时间线、操作按钮）
    ├── 滞留卡片自动高亮（黄色警告 + 卡住原因）
    └── 用户操作按钮（↻重试 / ⏮退回 / ⛔阻塞）
```
...
Python daemon，`launchctl` 管理。启动后：
1. 处理遗留 NOTIFY 文件
2. 执行一次 bootstrap（扫描 kanban，发现待处理任务写 NOTIFY）
3. 启动 fswatch 二进制监听 `.kanban/.notify/` 目录
4. 使用 `select()` 循环等待事件，每 60 秒超时执行一次定时 bootstrap
5. 收到 NOTIFY 文件后按前缀路由：writer-* → Writer，review-* → Reviewer

启动方式：`nohup /path/to/venv/python3 scripts/fswatch_daemon.py > /tmp/fswatch_out.log 2>&1 &`

### 7.3 Dashboard

FastAPI Web 应用（port 9119），提供全量看板可视化：
- **看板卡片**：按状态分组，显示标题/版本/时间/P级分布
- **自动化控制**：▶ 运行 / ⏸ 暂停 / ⏹ 停止（写入 automation_state.json）
- **Profiles 状态**：Writer/Reviewer/Watcher 状态跟随工作流（停止→已停止，暂停→已暂停，运行中→显示进程状态）
- **卡片详情**：点击查看完整信息（attempts、P级分布、revision_data、时间线、评论、自动修复状态）
- **滞留卡片**：超时未处理自动高亮黄色"+卡住原因"标签
- **用户操作**：每个滞留卡片详情弹窗底部提供 3 个按钮：↻重新触发、⏮退回待领取、⛔标记阻塞（含理由输入）

### 7.4 自动化控制状态

控制文件：`.kanban/.control/automation_state.json`
...
三种状态：
- **运行中** (running=true, paused=false): fswatch 正常处理 NOTIFY
- **已暂停** (running=true, paused=true): fswatch 跳过处理，保留 NOTIFY 文件
- **已停止** (running=false, paused=false): fswatch 跳过处理并清理 NOTIFY 文件

Dashboard 控制按钮写入此文件，fswatch daemon 定期读取。

### 7.5 NOTIFY 文件机制（事件驱动，零轮询）

Writer/Reviewer 完成工作后写 NOTIFY 文件到 `.kanban/.notify/` 目录：
```
writer-yaya-zhujiao    → 触发 Writer（yaya profile）
review-yaya-zhujiao    → 触发 Reviewer（yaya-reviewer profile）
```

NOTIFY 内容是纯文本（时间戳 + 触发原因），fswatch 二进制检测文件创建事件。Hermes 的 cron job 模式不再使用——写在配置文件里但实际由 fswatch 接管。
...
策略5：6轮循环上限。reviewer 判定 needs_revision 时 iteration_count+1（从0开始，首次提交为0），达到6时自动 blocked(reason: max_iterations_exceeded)。
...
文档 signed_off 后（P2 清零+终审签字后）触发自动评分，写入 quality_scores 表。
...
**AI 质量分（文档 signed_off 后独立执行，约 10K tokens，用 deepseek-chat 等廉价模型）**
...
系统当前使用 DeepSeek API，实际可靠并发为 **2**（gateway.log 已记录 22 次 HTTP 429 突发保护触发）。`MAX_CONCURRENCY=2` 在 `fswatch_daemon.py` 和 `writer.py` 中设置，Writer 和 Reviewer 共享同一并发池（通过 fswatch 的 `wait_for_slot()` 统一控制）。

架构支持最高 5 并发——扩量时只需将 `MAX_CONCURRENCY` 从 2 改为目标值，原子 CAS、git --<file> 隔离、per-task NOTIFY、busy_timeout 等基础设施均已就绪。但受限于 DeepSeek 突发保护，建议等待换用其他 provider 后再升并发。

配置要点：
- 每个项目只需 1 个 API Key——同一项目内 Writer 和 Reviewer 通过 fswatch 统一排队，不争抢并发槽位
- 当前单 Key 上限 2 并发，可配置第 2 个 Key 作为冗余备用
...
| AI 质量评分 | signed_off 后触发一次，约 10K tokens | 10K tokens/文档 |
...
生产环境跑几轮后，根据真实 token 消耗数据设置预算数字。当前阶段不对 token 做硬性控制，仅记录每轮实际消耗供后续校准。

### 9.4 逃生节省

同一 task 迭代第6轮时（iteration_count=5），writer 的 prompt 中嵌入提示：
"当前是第6轮修改机会。请重点检查前五轮 reviewer 标记的每一条问题是否全部修复。如果本轮仍然无法通过，iteration_count 将达 6，task 将自动 blocked。"
...
| task 循环6轮 | watcher | 暂停 + blocked + alert |
...
6. 实现 Watcher daemon（Python watchdog + 4个看门狗 + 竞态检查 + 多项目隔离）+ launchctl 管理

### Phase 2：核心循环（预计 2-3 天）
7. 实现 kanban SQLite 操作（含 iteration_number 字段）
8. 实现 Writer 写作流程（读取 task -> 写作 -> 自检 -> 提交 -> 通知 reviewer）
9. 实现 Reviewer 审核流程（被触发 -> 审核 -> 输出审计报告 -> 更新状态）
10. 实现 Writer->Reviewer 通知（HERMES_PROFILE=... hermes cron run）
11. 放置测试 task，跑通第一轮完整循环（含 p2_clearing 过渡）

### Phase 3：质量与评分（预计 2-3 天）
12. 实现质量评分体系（合规分 + AI 质量分 + 缺陷趋势分 + 分差>40 阻断 + 跨文档版本兼容性检查）
13. 实现自我进化机制（提议生成 + evolution_rules.yaml 更新 + scope 区分）

### Phase 4：打磨与扩展
14. 扩展到 Project Spark、Project Spark Adult
15. 进化机制验证（跑5-10轮后，比较 P0/P1 数量的趋势）
16. 如果进化有效，将 workflow package 成可复用的模板
17. 筹备开源+商业化分轨（开源版去除 project scope 规则，公开版含全部规则）
...
| revision | 修改中，合并旧 needs_revision 和 p2_clearing 状态 |
| re_review | 复审中，Writer 已修完等待 Reviewer 复查 |
| re_reviewing | 复审中，Reviewer claim 后的中间状态 |
| reviewing | 首次审核中，Reviewer claim 后的中间状态 |
| waiting_human_review | 审核通过等待人工评审，替代旧 approved 状态 |
| NOTIFY | 事件驱动文件（.kanban/.notify/），零轮询触发 Writer/Reviewer |
| automation_state | 自动化控制状态文件（.kanban/.control/automation_state.json） |
