...
项目本身服务于多个团队的文档质量需求。成熟后可以走开源+商业双轨：核心框架开源，增值服务（质量评分 API、预置规则包、质量趋势仪表盘、一键部署脚本）付费。核心壁垒是积累的审核规则集，不是架构代码。
...
- 第2步：按模板写初稿
- 第3步：5分钟自检——3极端场景走读 + 硬编码 grep 清单（见下）+ grep 每个跨系统引用是否在另一端有实现
...
8. 变更记录规范：版本号 bump、日期占位符未替换（占位符直接打回）
...
- failed：连续3轮 needs_revision 仍无法 passed -> 进入 blocked 状态
...
**阶段B——封版（signed_off）**
- 条件：1) approved 2) P2=0 清零（Writer修复，无需再次 reviewer 审核） 3) liufeng 人工终审签字
...
2. 变更记录最新条目日期与 git log 最后一次 commit 日期一致
...
### 4.3 多 Profile 架构 vs 现有工具的差异化优势
...
| 角色隔离 | 单 agent | 三 profile 独立模型/记忆/技能 |
...
- task_comments：task_id, author, content, created_at
...
| backlog | 待写作 | writer |
| drafting | 写作中 | writer |
| awaiting_review | 等待审核 | reviewer |
| needs_revision | 需要修改 | writer |
| approved | 审核通过 | reviewer |
| signed_off | 封版 | liufeng 手动 |
| blocked | 卡死 | watcher / liufeng |
...
Watcher 检测到异常时：暂停 writer/reviewer 的 cron job + 写 alert 文件到 .kanban/.alerts/ + launchctl 日志输出。
...
2. 死循环检测：iteration_count >= 3 -> blocked + alert
3. 数据丢失检测：kanban 状态与实际文件不一致 -> 检查 git 历史 -> 有则 checkout 恢复，无则回退状态
...
策略1：写作前加载自查清单。Writer 开始写作前，加载7项必备交付物清单 + 硬编码 grep 清单。

策略2：写作后实物验证。三项验证（git log / git diff / grep 关键概念）+ 调用 reusable-review-rules/wrapper.py 跑语法检查。
...
策略5：3轮循环上限。reviewer 判定 needs_revision 时 iteration_count+1，达到3轮自动 blocked。

策略6：Token 预算制度。Writer 和 reviewer 在各自会话结束前必须更新 kanban 的 tokens_spent 字段。超出 50% 自动 blocked。
...
AI 质量分与合规分相差超过40分时，触发"质量偏差警告"写入 watcher 日志，提示 owner 关注。
...
**评分的使用方式：**
- 不作为通过/不通过的决策依据（通过与否由 reviewer 判定）
- 作为 owner 观察质量趋势的工具
...
同一 task 迭代第3轮时，writer 的 prompt 中嵌入提示："当前是第3轮迭代。请重点检查前两轮 reviewer 标记的每一条问题是否全部修复。如果本轮仍然无法通过，task 将自动 blocked。"
...
| 文件丢失 | watcher | git checkout 恢复或回退状态 + alert |
| Token 超支 | watcher | 暂停 + blocked + alert |
| Watcher 自身挂掉 | launchctl | 自动重启 |
| 质量偏差（分差>40） | watcher | 写入 alert 日志 |

### 10.2 恢复流程

watcher 发现异常 -> 暂停 writer/reviewer 的 cron job -> 写 alert -> 输出日志 -> liufeng 介入分析 -> 按异常类型选择恢复路径。
...
1. 创建项目目录结构（已完成）
2. 创建 evolution_rules.yaml 初始模板
3. 实现 no_agent watch 脚本（30行 Python，扫描 SQLite）
4. 编写 reusable-review-rules/wrapper.py（调用 Vale + markdownlint）
5. 注册 writer 的 cron job

### Phase 2：核心循环（预计 2-3 天）
6. 实现 kanban SQLite 操作
...
10. 放置测试 task，跑通第一轮完整循环

### Phase 3：质量与监控（预计 2-3 天）
11. 实现 Watcher daemon（Python watchdog + 4个看门狗）
12. 实现质量评分体系（合规分 + AI 质量分 + 缺陷趋势分）
13. 实现自我进化机制（提议生成 + evolution_rules.yaml 更新）
14. launchctl 管理
...
- 预置规则包（基于几百轮审计经验积累的成熟规则集，按月订阅更新）
- 质量趋势仪表盘（跨文档/跨项目/跨时间可视化界面，不开源）
- 一键部署脚本（3分钟配置三 profile + 注册 cron + 启动 watcher，不开源）

### 12.4 两条线不冲突

- 开源用户：能自己跑起来，规则自己写，不依赖任何人
- 付费用户：半小时上线开箱即用，白嫖几百轮积累的规则，省下自己写规则和调参的时间

核心壁垒是审核规则集的质量和积累，不是架构代码。规则集是几百轮真实审计中积累的经验。

### 12.5 商业化最小路径

1. 现在阶段：就你一个用户，跑通芽芽的生产-审核循环，验证可行性
2. 第一阶段：稳定运行1个月，积累50条左右的进化规则 -> 发 GitHub + 完善部署文档
...
面向技术团队内部。S01-S16 子系统设计文档的自动化生产-审核-封版。当前 PRD 即为此层而写，所有设计决策（7项必备/8项必审/质量评分/自我进化）均在此层验证。
...
| 术语 | 含义 |
|------|------|
| Profile | Hermes 框架中的独立角色实例，有独立的模型、记忆、技能配置 |
