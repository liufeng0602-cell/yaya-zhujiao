...
- 文档进入 signed_off 后，触发质量评分（需在 P2 清零+终审签字后，确保评分反映最终质量）
...
blocked <- 任意状态（watcher发现异常/6轮上限/token超支/连续6轮审核不过）
```
...
| blocked | 卡死 | watcher / liufeng | 异常 / 6轮上限 / token 超支 / 连续6轮审核不过 |
...
2. 死循环检测：iteration_count >= 6（iteration_count 从 0 开始计数。Writer 首次提交时为 0，Reviewer 每次判定 needs_revision 时 +1。>= 6 时表示第 7 轮提交仍被退回，直接 blocked）-> blocked + alert
...
4. Token 超支检测（待 token 预算实测后启用）：tokens_spent > tokens_budget x 1.5 -> blocked + alert

### 7.3 Token 中间预警（待 token 预算实测后启用）
